...
<body>
    ...
    <h2 class="text-center text-green-800 font-semibold font-poppins text-3xl">Edit Data Wisata</h2>
...
<div class="container mx-auto my-10 ">
        <form class="flex justify-center items-center ml-8 lg:justify-center lg:items-center" method="POST" action="/{{ $action }}" enctype="multipart/form-data" >
            ...
            <div class="mb-6 ">
              ...
              <input class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg   w-full p-2.5" type="text" name="ket" class="form-control" value="{{ isset($data)?$data->ket:'' }}"
              ...
              <select id="tipe" name="tipe" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg form-control  w-full p-2.5"  value="{{ isset($data)?$data->tipe:'' }}"  >
                ...
                <option @if ($data['tipe'] == 'Beaches') selected @endif value="Beaches">Beaches</option>
                <option @if ($data['tipe'] == 'Deserts') selected @endif value="Deserts">Deserts</option>
                <option @if ($data['tipe'] == 'Mountains') selected @endif value="Mountains">Mountains</option>
                <option @if ($data['tipe'] == 'Waterfall') selected @endif value="Waterfall">Waterfall</option>
                <option @if ($data['tipe'] == 'Houseboats') selected @endif value="Houseboats">Houseboats</option>
                <option @if ($data['tipe'] == 'Countryside') selected @endif value="Countryside">Countryside</option>
                <option @if ($data['tipe'] == 'Camping') selected @endif value="Camping">Camping</option>
                <option @if ($data['tipe'] == 'Tropical') selected @endif value="Tropical">Tropical</option>

              </select>

              <label for="maps" class="block mb-2 text-sm font-medium mt-2">Link Maps</label>
              <input class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg   w-full p-2.5" type="text" name="maps" class="form-control" value="{{$data->maps}}">
              ...
              <textarea
                    ...
                    <a class="font-bold">Gambar Tersedia
                        <img src="{{ asset('storage/' . $data->img)}}"  class="object-scale-down">
                    </a>
              <label class="block mb-2 text-sm font-medium text-gray-900 " for="img">Upload Gambar</label>
                ...
                <input class="w-full py-1 text-sm  jus text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 " id="img" name="img" type="file"
                @error('img') is-invalid @enderror onchange="previewimg()">
                @error('img')
                    <div class="invalid-feedback text-xs text-red-500">{{$message}}</div>
                @enderror
