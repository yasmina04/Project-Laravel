...
<body>
    <h1 class="text-green-700 font-poppins justify-start text-2xl font-semibold my-10 mx-auto ml-8">List Event</h1>

    <button class="bg-green-700 rounded-lg ml-8 mx-auto px-4 py-2">
        <a href="/event/create" class="text-white">Tambah</a><br />
    </button>
        ...
        <table class="w-full text-sm border-b text-left mb-6 text-gray-500 ">
            ...
            <tbody>
                ...
                <tr class=" border-b ">
                    ...
                    <td class="px-6 py-4">
                        <img src="{{ asset($d->img) }}" class="object-scale-down w-30 h-28">
                    </td>

                    <td class="px-6 py-4 ">
                        <a href="{{ route('editForm',$d->id) }}" class="font-semibold mr-5 mx-auto text-green-700 rounded-lg">Edit</a>
                        <form method="post" action="{{ route('deleteForm',$d->id) }}" style="display:inline" onsubmit="return confirm('Yakin hapus?')">
                            @csrf
                            @method('POST')
      ...
      <div class="col-3">
        ...
        <table class="table table-bordered table-striped">
          ...
          <tr>
            ...
            <td>{{ $d->tanggal }}</td>
            <td>{{ $d->detail }}</td>
            <td><img src="{{ asset($d->img) }}" style="height:120px; width:200px"/></td>
            <td>
              <a href="{{ route('editForm',$d->id) }}" class="btn btn-primary">Edit</a>
	                <form method="post" action="{{ route('deleteForm',$d->id) }}" style="display:inline" onsubmit="return confirm('Yakin hapus?')">
	                @csrf
	                @method('POST')
	                <button class="btn btn-danger">Hapus</button>
	              </form>
              <!--<a href="{{ route('deleteForm',$d->id) }}" class="btn btn-danger">Hapus</a> -->
            </td>
          </tr>
          @endforeach
        </table>
